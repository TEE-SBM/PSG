...
def save_results(result, dataset, model_cfg, optim_cfg, seed, out_path="results/"):
    ...
    path = get_path([dataset, var_to_str(model_cfg_), var_to_str(optim_cfg)], out_path=out_path)
...
def load_results(dataset, model_cfg, optim_cfg, seed, out_path="results/"):
    ...
    model_cfg_ = {k: model_cfg[k] for k in key_}
    path = get_path(
        [dataset, var_to_str(model_cfg_), var_to_str(optim_cfg)], out_path=out_path
    )
    f = os.path.join(path, f"seed_{seed}.p")
