...
def load_dataset(dataset="yacht", test_size=0.4, data_path="data/"):
    if not os.path.exists(data_path):
        ...
        try:
            if dataset == "amazon":
                ...
                X_train = np.load(os.path.join(data_path, dataset, "X_train.npy"))
                y_train = np.load(os.path.join(data_path, dataset, "y_train.npy"))
                X_val_test = np.load(os.path.join(data_path, dataset, "X_test.npy"))
                y_val_test = np.load(os.path.join(data_path, dataset, "y_test.npy"))
        ...
        X_test = np.load(os.path.join(data_path, "iwildcam/X_test.npy"))
        y_test = torch.tensor(np.load(os.path.join(data_path, "iwildcam/y_test.npy")))
        ...
        X_y_train = torch.load(os.path.join(data_path, "tiny-imagenet/pca_train_features.pt"), weights_only=False)
        X_y_val = torch.load(os.path.join(data_path, "tiny-imagenet/pca_val_features.pt"), weights_only=False)
        X_y_test = torch.load(os.path.join(data_path, "tiny-imagenet/pca_test_features.pt"), weights_only=False)
        
        X_train = X_y_train["features"]
        print(X_train.shape)
        y_train = X_y_train["labels"]
        
        X_val = X_y_val["features"]
        y_val = X_y_val["labels"]
        
        X_test = X_y_test["features"]
        y_test = X_y_test["labels"]
