...
from src.optim.smoothing import get_smooth_weights_sorted
...
class Objective:
    def __init__(
        ...
        self.shift_cost = self.n * shift_cost if penalty == "l1" else shift_cost
        self.penalty = penalty

    def get_batch_general_loss(self, w):
        with torch.no_grad():
            return torch.mean(self.loss(w, self.X, self.y))
    
    def get_batch_spectral_loss(self, w): # is not involed in optimization step, use for evaluation.
        with torch.no_grad():
    ...
    def get_batch_loss(self, w, include_reg=True): # is not involed in optimization step, use for evaluation.
        with torch.no_grad():
            sorted_losses = torch.sort(self.loss(w, self.X, self.y), stable=True)[0]
            n = self.n
            if self.lp_reg:
                ...
                risk = torch.dot(self.sigmas, sorted_losses)
            if self.lp_reg and include_reg:
                # Edited by W Wang
                if self.reg_type == "l1":
                    risk +=  self.lp_reg * torch.norm(w, p=1) / self.n
                else:
                    risk +=  0.5*self.lp_reg * torch.norm(w)**2 / self.n
            return risk
    ...
    def get_batch_subgrad_oracle(self, w, idx=None, include_reg=True):
        ...
        if self.lp_reg and include_reg:
            if self.reg_type == "l1":
                g += self.lp_reg * w.sign().detach() / self.n
            else:
                g += self.lp_reg * w.detach() / self.n
        return g
    ...
    def get_indiv_prox_loss(self, w, stepsize, i):
        real_lp_reg = self.lp_reg / self.n
        with torch.no_grad():
            ...
            return prox_with_l1reg(
                w, self.X[i], self.y[i], stepsize, prox_op, real_lp_reg
            )
...
def prox_with_l1reg(w, x, y, stepsize, prox, l1reg):
    scaled_stepsize = stepsize / (1 + stepsize * l1reg)
    scaled_w = w / (1 + stepsize * l1reg)
